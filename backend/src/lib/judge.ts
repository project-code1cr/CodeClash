...
interface ProblemJudgeSpec {
  methodName: string;
  returnType: "int" | "bool" | "intArray";
  tests: JudgeCase[];
}
...
const PROBLEM_SPECS: Record<string, ProblemJudgeSpec> = {
  "Two Sum": {
    methodName: "twoSum",
    returnType: "intArray",
    tests: [
...
const normalizeValue = (value: unknown, ignoreOrder = false): string => {
...
const mapExecutionFailure = (text: string): JudgeVerdict => {
  if (/time limit|timed out|timeout/i.test(text)) return "TLE";
  if (/memory limit|out of memory|bad_alloc|cannot allocate/i.test(text)) return "MLE";
  return "RUNTIME_ERROR";
};

const mapCommandErrorToVerdict = (text: string): JudgeVerdict => {
  if (/syntaxerror|compile|error:/i.test(text)) return "COMPILATION_ERROR";
...
const serializeArg = (arg: unknown, lang: JudgeLanguage): string => {
  ...
  if (Array.isArray(arg)) {
    if (lang === "javascript") return JSON.stringify(arg);
    if (lang === "java") return `new int[]{${arg.join(",")}}`;
...
const buildCppProgram = (
  code: string,
  methodName: string,
  args: unknown[],
  returnType: ProblemJudgeSpec["returnType"]
): string => {
  const cppArgDeclarations: string[] = [];
  const cppCallArgs: string[] = [];
...
const executeOnPiston = async (
  lang: JudgeLanguage,
  sourceCode: string
): Promise<{ stdout: string; stderr: string; compileOutput: string; code: number }> => {
  if (!PISTON_URL) {
  ...
  const data = (await response.json()) as {
    ...
    compile?: { output?: string; stderr?: string; stdout?: string };
    message?: string;
    error?: string;
  };
  ...
  return {
    ...
    compileOutput:
      data.compile?.stderr || data.compile?.output || data.compile?.stdout || "",
...
const executeLocally = async (
  ...
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "codeclash-judge-"));
  ...
  try {
    ...
    throw new Error(
      "Java judge unavailable locally. Configure PISTON_URL to run Java submissions."
    );
  } finally {
